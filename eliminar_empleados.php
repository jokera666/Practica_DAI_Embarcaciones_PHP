...
$error = 0;
	...
	{
		$consulta = "DELETE FROM EMPLEADOS WHERE Id_Empleado = '".$array_borrados[$i]."'";
		...
		if($error==0)
		{
			if($tipo=="Empleado") echo "<script>alert('El (los) Empleado(s) se ha(n) eliminado correctamente.'); document.location=('./indexEmpleadoAJAX.php');</script>";
			else echo "<script>alert('El (los) Empleado(s) se ha(n) eliminado correctamente.'); document.location=('./indexAdmin.php');</script>";
		}
