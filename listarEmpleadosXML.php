<?php 
	...
	foreach ($rows as $fila)
	{
		...
		echo "<empleados>";
		echo '<borrar><![CDATA[<input type="checkbox" name="checkBorrar[]" value="'.$id_empleado.'"> <i class="fa fa-trash fa-2x"/>]]></borrar>';
		echo '<modificar><![CDATA[<a href="formModificar_empleados.php?idempleado='.$id_empleado.'&correo='.$cp.'"><i class="fa fa-edit fa-2x"/></a>]]></modificar>';
		echo '<id>'.$id_empleado.'</id>';
		echo '<tipo>'.$tipo.'</tipo>';		
		echo '<dni>'.$dni.'</dni>';
